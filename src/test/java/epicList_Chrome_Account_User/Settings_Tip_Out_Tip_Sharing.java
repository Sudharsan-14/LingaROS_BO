...
public class Settings_Tip_Out_Tip_Sharing 
{
	...
	//	Thread.sleep(3000);
		...
		if(wait.until(ExpectedConditions.visibilityOf(ele)).getText().equalsIgnoreCase("Tip Out/Tip Sharing Updated Sucessfully"))
		{
	...
	//List of Tip Share per hour list
		List<WebElement> list=driver.findElements(By.xpath(excel.getData(3, 2316, 1)));
		
		int listSize=list.size();
		
		for(int i=1;i<=listSize;i++)
		{
			//Remove the Tip Share per hours list
			driver.findElement(By.xpath("//table[@class='table table-hover table-nomargin table-bordered nopadding ng-table-responsive']/tbody/tr["+listSize+"]/td[6]/div/a")).click();
		}
		
		
		Thread.sleep(1500);
		//Disable Tip out Percentage
		driver.findElement(By.xpath(excel.getData(3, 2076, 1))).click();
	...
	//	Thread.sleep(3000);
		...
		//Check whether the Tip Out/Tip Sharing Settings Saved or Not
		if(wait1.until(ExpectedConditions.visibilityOf(ele1)).getText().equalsIgnoreCase("Tip Out/Tip Sharing Updated Sucessfully"))
		{
			test.log(LogStatus.PASS, "Tip Out Sharing Updated Successfully");
		}
		else
	...
	public void Manual_Tip_Sharing_Settings_Save_Settings(WebDriver driver) throws Exception
	{
		...
		List<WebElement> list=driver.findElements(By.xpath(excel.getData(3, 2315, 1)));
		int listSize=list.size()-1;
		
		for(int i=1;i<=listSize;i++)
		{
			//Remove Roles from Manual Tip
			driver.findElement(By.xpath("//select[@ng-model='tipOutSharing.tipSharingRoles']/../div/ul/li[1]/a")).click();
		}
		
		Thread.sleep(3000);
		//Disable Manual Tip Sharing 
		driver.findElement(By.xpath(excel.getData(3, 2088, 1))).click();
