...
public class Enterprise_Loyalty_Report {
	...
	float unknownValue = 00;
		...
		driver.findElement(By.xpath(excel.getData(2, 398, 1))).sendKeys(Utility.getReportPropertyUser("Date_Range_From"));
		...
		driver.findElement(By.xpath(excel.getData(2, 399, 1))).sendKeys(Utility.getReportPropertyUser("Date_Range_To"));
...
/*	    	//Replace all commo's with empty space
	    	String expected1 = Utility.getReportPropertyUser("Loyalty_Customer_Name").replace(",", "");
	    	
	    	//Convert the String value of the Sale_Report_Sale_Amount element into float value
	    	float expect1 = Float.parseFloat(expected1);  
	    		        	
	    	//Replace all commo's with empty space
	    	String expected2 = Utility.getReportPropertyUser("Loyalty_Sign_upDate").replace(",", "");
	    	
	    	Float.parseFloat(expected2);  
	    	
	    	//Replace all commo's with empty space
	    	String expected3 = Utility.getReportPropertyUser("Loyalty_Last_Redemption_Date").replace(",", "");
	    	
	    	Float.parseFloat(expected3);  

	    	//Replace all commo's with empty space
	    	String expected4 = Utility.getReportPropertyUser("Loyalty_Last_AccumulationDate").replace(",", "");
	    	
	    	Float.parseFloat(expected4);  
	    	
	    	//Replace all commo's with empty space
	    	String expected5 = Utility.getReportPropertyUser("Total_Redeemed_LoyaltyPoints").replace(",", "");
	    	
	    	Float.parseFloat(expected5);                              
	        
	    	//Replace all commo's with empty space
	    	String expected6 = Utility.getReportPropertyUser("Total_LoyaltyPoints").replace(",", "");
	    	
	    	Float.parseFloat(expected6);                  
	        //Check weather the Sale Amount Report is correct or not
	        if(driver.findElement(By.xpath("//div[@id='main-container']/div[2]/div/div/div[2]/div[2]/div/div/div/div/div[7]/div/table/tbody/tr["+rows.size()+"]/td[2]/span")).getText().equals(Utility.getReportPropertyUser("Sale_Report_Sale_Amount")))
	        {
