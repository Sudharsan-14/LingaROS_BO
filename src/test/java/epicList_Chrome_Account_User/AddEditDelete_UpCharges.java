...
/*		//Click the Products/Items option
		...
		driver.get(Utility.getProperty("baseURL")+Utility.getProperty("store_Id1")+"upcharge/list");
		Thread.sleep(5000);
		try
		{
		//Check Up Charges page opened or not
		if(driver.findElement(By.xpath(excel.getData(3, 2312,1))).getText().equalsIgnoreCase("UpCharges"))
		{
	...
	public void Upcharges_method_editUpCharges(WebDriver driver) throws Exception
	{
		...
		//Click the Edit icon
		Thread.sleep(8000);driver.findElement(By.cssSelector(excel.getData(3, 41, 4))).click();
		Thread.sleep(3000);
	...
	public void Upcharges_method_deleteUpCharges(WebDriver driver) throws Exception
	{
		...
		if(wait.until(ExpectedConditions.visibilityOf(ele)).getText().equalsIgnoreCase("Upcharge Inactivated Successfully"))
		{
